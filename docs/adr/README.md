...
146 architecture decision records. Listed in numeric order. "Status" is the
...
| [0164-fusion-deliberation-boundary.md](./0164-fusion-deliberation-boundary.md) | ADR 0164 — OpenRouter Fusion is a managed clone of Chimera's existing local deliberation (`cross_critique`, alignment ceremony, primary+escalator critic gate); modeled as an optional default-OFF PLAN-phase idea-divergence primitive and **explicitly walled off** from the critic gate, alignment ceremony, and `submit_pr.validate()` — its opacity breaks replayable fail-closed adjudication and its server-side spend breaks `cost_estimate` metering. Analysis-only; no integration shipped. (index gap: 0147–0163 not backfilled) | Proposed (2026-06-04) |
