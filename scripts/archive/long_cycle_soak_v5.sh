#!/usr/bin/env bash
# scripts/long_cycle_soak_v5.sh — focused remediation retry
#
...
soak_refuse_concurrent "long_cycle_soak_v5.sh" || exit $?
...
BRANCH="chimera-soak/v5-$STAMP"
WORKTREE="${WORKTREE:-$REPO_ROOT/../chimera-soak-v5-$STAMP}"
...
LOG="$REPO_ROOT/state/long_cycle_v5_2026-05-20.log"
...
# ── pre-flight ─────────────────────────────────────────────────
log "─────────────────────────────────────────────────────────────"
log "long_cycle_soak_v5.sh start"
log "  branch         = $BRANCH"
...
git config --worktree remote.origin.pushurl \
    "no-push://disabled-for-soak-v5-$STAMP" 2>&1 | tee -a "$LOG" || true
...
log "─────────────────────────────────────────────────────────────"
log "long_cycle_soak_v5.sh complete"
