#!/usr/bin/env bash
# scripts/long_cycle_soak_v4.sh — focused remediation retry
#
...
soak_refuse_concurrent "long_cycle_soak_v4.sh" || exit $?
...
BRANCH="chimera-soak/v4-$STAMP"
WORKTREE="${WORKTREE:-$REPO_ROOT/../chimera-soak-v4-$STAMP}"
...
LOG="$REPO_ROOT/state/long_cycle_v4_2026-05-20.log"
...
# ── pre-flight ─────────────────────────────────────────────────
log "─────────────────────────────────────────────────────────────"
log "long_cycle_soak_v4.sh start"
log "  branch         = $BRANCH"
...
git config --worktree remote.origin.pushurl \
    "no-push://disabled-for-soak-v4-$STAMP" 2>&1 | tee -a "$LOG" || true
...
log "─────────────────────────────────────────────────────────────"
log "long_cycle_soak_v4.sh complete"
