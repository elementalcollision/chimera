#!/usr/bin/env bash
# scripts/long_cycle_soak_v11.sh — focused remediation retry
#
...
soak_refuse_concurrent "long_cycle_soak_v11.sh" || exit $?
...
BRANCH="chimera-soak/v11-$STAMP"
WORKTREE="${WORKTREE:-$REPO_ROOT/../chimera-soak-v11-$STAMP}"
...
LOG="$REPO_ROOT/state/long_cycle_v11_${STAMP}.log"
...
# ── pre-flight ─────────────────────────────────────────────────
log "─────────────────────────────────────────────────────────────"
log "long_cycle_soak_v11.sh start"
log "  branch         = $BRANCH"
...
git config --worktree remote.origin.pushurl \
    "no-push://disabled-for-soak-v11-$STAMP" 2>&1 | tee -a "$LOG" || true
...
# Inbox — Soak v11 phase 1 (investigation only, engines off)
...
log "─────────────────────────────────────────────────────────────"
log "long_cycle_soak_v11.sh complete"
