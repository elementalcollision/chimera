#!/usr/bin/env bash
# scripts/long_cycle_soak_v8.sh — focused remediation retry
#
...
soak_refuse_concurrent "long_cycle_soak_v8.sh" || exit $?
...
BRANCH="chimera-soak/v8-$STAMP"
WORKTREE="${WORKTREE:-$REPO_ROOT/../chimera-soak-v8-$STAMP}"
...
LOG="$REPO_ROOT/state/long_cycle_v8_${STAMP}.log"
...
# ── pre-flight ─────────────────────────────────────────────────
log "─────────────────────────────────────────────────────────────"
log "long_cycle_soak_v8.sh start"
log "  branch         = $BRANCH"
...
git config --worktree remote.origin.pushurl \
    "no-push://disabled-for-soak-v8-$STAMP" 2>&1 | tee -a "$LOG" || true
...
# Inbox — Soak v8 phase 1 (investigation only, engines off)
...
log "─────────────────────────────────────────────────────────────"
log "long_cycle_soak_v8.sh complete"
