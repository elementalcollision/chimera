#!/usr/bin/env bash
# scripts/long_cycle_soak_v33.sh — Chip T1.2 from post-baseline priorities
#
# Charter source: mind/research/post-baseline-development-priorities-2026-05-24.md
# (landed via PR #57). Closes Failure mode A from the smoke baseline
# (PR #56): multi-session (4/4 wrong) and knowledge-update (4/4 wrong)
# categories at 20% accuracy. Root cause: the dialectic prompt assembles
# all sessions into one markdown blob but does NOT direct the model to
# integrate cross-session facts or prefer later sessions when earlier
# facts are contradicted. Expected delta: +30pp on multi-session +
# knowledge-update (conservative estimate; range 0-60pp).
#
# Atomic-op class: extend-prompt-template + add-test + add-adr.
# Sibling templates:
#   - Existing _DIALECTIC_PROMPT constant at chimera/a2a/dialectic.py:156
#   - ADR 0133 (dialectic API, the surface this chip extends).
# 4 files total — within atomic shape since the change is one prompt
# string + one test + one ADR + one README index line.

set -uo pipefail

# shellcheck disable=SC1091
. "$(dirname "$0")/_soak_common.sh"
soak_refuse_concurrent "long_cycle_soak_v33.sh" || exit $?
...
BRANCH="chimera-soak/v33-$STAMP"
WORKTREE="${WORKTREE:-$REPO_ROOT/../chimera-soak-v33-$STAMP}"
...
LOG="$REPO_ROOT/state/long_cycle_v33_${STAMP}.log"
...
# ── pre-flight ─────────────────────────────────────────────────
log "─────────────────────────────────────────────────────────────"
log "long_cycle_soak_v33.sh start — Chip T1.2 (temporal-aware dialectic)"
log "  branch         = $BRANCH"
log "  worktree       = $WORKTREE"
log "  phase1 cap     = \$$PHASE1_CAP_USD (engines OFF — operator focus)"
log "  phase2 cap     = \$$PHASE2_CAP_USD (engines ON, session mode)"
log "  charter        = extend _DIALECTIC_PROMPT with cross-session instructions"
log "─────────────────────────────────────────────────────────────"
...
# ── set up worktree ────────────────────────────────────────────
# NOTE for v34+ template authors: keep the soak_sync_main_from_origin
...
git config --worktree remote.origin.pushurl \
    "no-push://disabled-for-soak-v33-$STAMP" 2>&1 | tee -a "$LOG" || true
...
# Phase-1 INBOX — v33 ships Chip T1.2 from post-baseline priorities.
# 4 files: chimera/a2a/dialectic.py, tests/test_dialectic.py,
# docs/adr/0136-temporal-aware-dialectic.md, docs/adr/README.md.
# ADR status: Proposed (promotion gate: smoke shows multi-session +
# knowledge-update each move ≥20pp).
cat > "$WORKTREE/mind/INBOX.md" <<'INBOX_EOF'
# Inbox — Soak v33 phase 1 (investigation only, engines off)

**Chip T1.2** from the post-baseline priorities doc (landed via PR #57).
**Atomic op**: extend-prompt-template + add-test + add-adr.
**Target**: Extend `_DIALECTIC_PROMPT` in `chimera/a2a/dialectic.py`
with explicit cross-session instructions to close Failure mode A
(multi-session at 20% + knowledge-update at 20% in the smoke baseline).

**Background**: LongMemEval smoke baseline (PR #56, 30 items) showed
multi-session (4/4 wrong) and knowledge-update (4/4 wrong) categories
both at 20% accuracy. Per-item review attributed this to Failure mode
A: the dialectic prompt assembles all sessions into one markdown blob
but does NOT direct the model to integrate cross-session facts or to
prefer later sessions when earlier ones are contradicted. Expected
delta: +30pp on those two categories (conservative; range 0-60pp).

The template:
- **`_DIALECTIC_PROMPT`** at `chimera/a2a/dialectic.py:156` — the
  format-string the dialectic API hands to the model.
- **ADR 0133** (dialectic API) is the parent surface this extends.
- **`tests/test_dialectic.py`** has the existing tests; new test
  belongs at the END (extend, do NOT create a new file).
- **`docs/adr/README.md`** is the index — append a row, bump count
  from 132 to 133.

## Phase 1 tasks (investigation)

- [ ] Read `_DIALECTIC_PROMPT` at `chimera/a2a/dialectic.py:156`
  (extends to ~line 175 with the closing triple-quote). Note the
  section structure (peer card, decisions, beliefs, KFM) and the
  placement of the initial instructions before "Question:".
- [ ] Read at least 2 existing tests in `tests/test_dialectic.py` to
  understand the assertion style for prompt-content checks.
- [ ] Read ADR 0133 at `docs/adr/0133-dialectic-api.md` to confirm
  the dialectic API surface and the locked-design table style for
  the new ADR 0136.
- [ ] Spec the change. Write to
  `mind/research/v33-temporal-dialectic-design.md`. The file MUST end
  with a section whose heading is EXACTLY: `## READY-FOR-REMEDIATION`

  Under that heading:
    (a) The exact NEW text to add to `_DIALECTIC_PROMPT`. Two
        sentences max:
        1. "When the question requires information from multiple
           sessions, integrate facts across the entire history."
        2. "When a fact stated in an earlier session is contradicted
           by a later session, prefer the later session."
    (b) The placement: append to the initial instructions block,
        BEFORE the "Question:" line. Keep the existing "Use ONLY the
        grounding below" and "If the grounding doesn't support an
        answer, say so honestly" sentences intact and contiguous.
    (c) The test assertion (one line of pseudocode): verify the new
        cross-session phrasing appears in `build_dialectic_prompt(...)`
        output via substring check on both sentences.
    (d) The ADR 0136 skeleton: status `Proposed`, locked-design table
        with rows for (Surface, Charter scope, Promotion gate,
        Out of scope). Reference ADR 0133.
...
- ONE edit to `chimera/a2a/dialectic.py`: extend the
  `_DIALECTIC_PROMPT` string with the two cross-session sentences
  appended to the initial instructions block.
- ONE new test in `tests/test_dialectic.py` (at the end) verifying
  both new sentences appear in `build_dialectic_prompt(...)` output.
- ONE new ADR file `docs/adr/0136-temporal-aware-dialectic.md`
  (status `Proposed`).
- ONE new index row in `docs/adr/README.md` + bump count 132 → 133.
...
- Re-run tests post-commit, write the result line to
  `mind/research/v33-temporal-dialectic-remediation.md` under
  `## Test results`.
...
witness panel from this task text):
  ...
  1. SCOPE: FOUR files — `chimera/a2a/dialectic.py` (one prompt
     ...
     `docs/adr/0136-temporal-aware-dialectic.md` (new, Proposed),
  ...
  2. SEMANTICS: the extended `_DIALECTIC_PROMPT` still renders via
     ...
     placeholders (`{peer_name}`, `{question}`, `{peer_card_block}`,
     `{decisions_block}`, `{beliefs_block}`, `{safe_name}`,
     `{kfm_block}`) MUST remain valid; existing tests MUST still pass.
  3. PATTERN: two new sentences appended to the initial instructions
     block (after the "say so honestly" sentence, before "Question:").
     Mirror the existing prose style (declarative, second-person, no
     markdown).
  4. NO modification of `_UNKNOWN_PEER_PROMPT` (different code path;
     out of scope for cross-session reasoning).
  5. NO new helper functions; the change is one string + one test
     + one ADR + one README row.
  6. NO new CLI flags, env knobs, or behavior changes elsewhere.
  7. The change must NEVER raise on benign inputs (string change only;
     no logic added).
  8. NO new dependencies. Stdlib + existing deps only.

OVERSHOOT TRAPS the panel should reject:

  - **Rewriting the entire prompt template** (charter #3 — append two
    sentences, do NOT restructure sections).
  - **Touching the answer-side prompt** (we only own the assembled
    grounding layer; the answerer model decides how to use it).
  - **Modifying `_UNKNOWN_PEER_PROMPT`** (charter #4).
  - **Adding env knobs** like `CHIMERA_TEMPORAL_DIALECTIC=1` —
    behavior change is unconditional (it is a prompt addition, not a
    runtime toggle).
  - **ADR status `Accepted`** — must be `Proposed` until promotion
    gate (post-Tier-1 sweep) clears it.
  - **Combining T1.3 (preference-aware)** — that is a separate chip;
    different sentence about user preferences, different ADR (0137).
...
This is Chip T1.2 (post-baseline critical path: T1.1 done → T1.2
this soak → T1.3 next → T1.4 sweep → T2.1 retrieval). Tight scope
is what makes the chip queue work.

INBOX_EOF

log "phase-1 INBOX seeded (v33 Chip T1.2: temporal-aware dialectic)"
...
cat > "$WORKTREE/mind/INBOX.md" <<'INBOX_EOF'
# Inbox — Soak v33 phase 2 (remediation, engines on)

Phase 1's design is in
`mind/research/v33-temporal-dialectic-design.md` under
...
CHARTER (v4.112 charter extraction will pass this to the witness panel):
  ...
  1. SCOPE: FOUR files — `chimera/a2a/dialectic.py` (one prompt
     extension), `tests/test_dialectic.py` (one new test),
     `docs/adr/0136-temporal-aware-dialectic.md` (new, Proposed),
     `docs/adr/README.md` (one new row + count bump 132→133).
  2. SEMANTICS: extended `_DIALECTIC_PROMPT` still renders via
     `build_dialectic_prompt(ctx, question)`; existing format-string
     placeholders remain valid; existing tests still pass.
  3. PATTERN: two new sentences appended to initial instructions
     block, before "Question:". Declarative, second-person, no markdown.
  4. NO modification of `_UNKNOWN_PEER_PROMPT`.
  5. NO new helpers.
  6. NO new CLI flags or env knobs.
  7. Never raises on benign inputs.
  8. Stdlib + existing deps only.

## Phase 2 tasks

- [ ] Re-read the design from phase 1.
- [ ] Append the two cross-session sentences to `_DIALECTIC_PROMPT`
  at `chimera/a2a/dialectic.py:156` (initial instructions block).
- [ ] Add ONE new test in `tests/test_dialectic.py` (at end) verifying
  both new sentences appear in `build_dialectic_prompt(...)` output.
- [ ] Create `docs/adr/0136-temporal-aware-dialectic.md` (Proposed,
  locked-design table style, references ADR 0133).
- [ ] Add ONE new row in `docs/adr/README.md` + bump count 132→133.
...
OVERSHOOT TRAPS the panel should reject:
  ...
  - **Rewriting the entire prompt template** (append two sentences only).
  ...
  - **Combining T1.3 (preference-aware)** — separate chip, separate ADR (0137).
  - **Commit message rooted-path discipline** (v4.115).
  - **Committing with red tests** (v23 failure mode).
  - **Lying-by-honesty**: shipping with failure counts.

This is Chip T1.2 (post-baseline tier 1, second of four). Single
...
SOFT_SENTINEL_ALLOWED_FILES="chimera/a2a/dialectic.py tests/test_dialectic.py docs/adr/0136-temporal-aware-dialectic.md docs/adr/README.md"
...
log "─────────────────────────────────────────────────────────────"
log "long_cycle_soak_v33.sh complete"
...
log "        cat mind/research/v33-temporal-dialectic-design.md"
log "        cat mind/research/v33-temporal-dialectic-remediation.md"
