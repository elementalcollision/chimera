#!/usr/bin/env bash
# scripts/long_cycle_soak_v37.sh — micro-soak, N=5 fan-out classification.
#
# Charter: builds on v36's N=1 CONVERGES result (PR #115 / #117 postmortem)
# by fanning out to FIVE LoCoMo F2 temporal-reasoning regression items
# (items 1-5 by item_id sort order, no item-selection discretion).
# Tests whether the v36 atomic shape scales without re-introducing the
# v35 multi-item ACT-phase-budget failure mode. Deliverable: one research
# note containing five labelled paragraphs. Phase 2 commits the research
# note only — R1, no code changes.
#
# Built-in consistency check: item #1 is `conv-26::qa14` (the same item
# v36 classified as H2). v37 MUST classify it independently from the F1/F2
# data — if the same evidence again lands H2, that's a per-run consistency
# signal. The v36 prior label is referenced in INBOX only as a pointer for
# the operator's later sanity check, NOT as input to the classification.
#
# Post-cascade hardening (PRs #103–#110, #113, #118, #119) is inherited by
# virtue of cloning v36's scaffold: forward-progress watchdog, task-
# completion watchdog, ACT-phase budget enforcement, pre-commit scope
# check, ADR 0141 worktree detector, SQLite thread-affinity fix, phase-1
...
# design note mind/research/v37-micro-soak-design-2026-05-28.md).

set -uo pipefail

# shellcheck disable=SC1091
. "$(dirname "$0")/_soak_common.sh"
soak_refuse_concurrent "long_cycle_soak_v37.sh" || exit $?
...
BRANCH="chimera-soak/v37-$STAMP"
WORKTREE="${WORKTREE:-$REPO_ROOT/../chimera-soak-v37-$STAMP}"
...
LOG="$REPO_ROOT/state/long_cycle_v37_${STAMP}.log"
...
# ── pre-flight ─────────────────────────────────────────────────
log "─────────────────────────────────────────────────────────────"
log "long_cycle_soak_v37.sh start — micro-soak (N=5 fan-out temporal classification)"
log "  branch         = $BRANCH"
log "  worktree       = $WORKTREE"
log "  phase1 cap     = \$$PHASE1_CAP_USD (engines OFF — investigation)"
log "  phase2 cap     = \$$PHASE2_CAP_USD (engines ON, commit-only)"
log "  charter        = classify FIVE LoCoMo F2 temporal regression items (sort-first); commit the note"
log "─────────────────────────────────────────────────────────────"
...
git config --worktree remote.origin.pushurl \
    "no-push://disabled-for-soak-v37-$STAMP" 2>&1 | tee -a "$LOG" || true
...
# Phase-1 INBOX — v37 atomic charter: classify FIVE LoCoMo F2 temporal-
# reasoning regression items (items 1-5 by item_id sort order).
cat > "$WORKTREE/mind/INBOX.md" <<'INBOX_EOF'
# Inbox — Soak v37 phase 1 (investigation only, engines off)

**Chip**: classify FIVE LoCoMo F2 temporal-reasoning regression items
(items 1-5 by item_id sort order).
...
35.42% (F2). 19 items went from F1-right to F2-wrong. v36 (PR #115)
classified ONE item (`conv-26::qa14`) as H2 (context-budget dilution).
v37 fans out to FIVE items, classified in item_id sort order.

**This chip classifies items #1 through #5 by item_id sort order.**

Item #1 (`conv-26::qa14`) MUST be re-classified independently —
do NOT copy v36's label. This is a consistency check: same evidence
should produce the same label.

**Three locked hypotheses** (same as v36):
...
- v36 prior classification: `mind/research/v36-locomo-temporal-one-item-classification.md` (for item #1 consistency check; do NOT copy the label)
...
- [ ] **Sort by item_id ascending; take the first 5.** No item-
...
- [ ] For EACH of the 5 items, in order:
  ...
  - Each paragraph starts with a heading like `## Item N: <item_id> → <label>`
- [ ] When ALL 5 are classified, append a final section
  `## READY-FOR-REMEDIATION` with content:
  "Classified items <id1>..<id5> as <labels>. R1 — no code change.
   This chip's atomic deliverable is the five classifications; phase 2
   commits this research note only."
- [ ] Deliverable file path: `mind/research/v37-locomo-temporal-5-item-classification.md`

Do NOT classify more than 5 items. Do NOT skip items. Do NOT propose
adapter changes. Do NOT modify ADR 0142. Do NOT copy v36's label for
item #1 — produce it independently from the F1/F2 data.

CHARTER for phase 2:

  1. SCOPE: ONE file — `mind/research/v37-locomo-temporal-5-item-classification.md`.
  ...
  6. Commit message: `[agent]` prefix + "classify items <id1>..<id5> as <labels>".

OVERSHOOT TRAPS the panel should reject:
  - Classifying more than 5 items
  ...
  - Copying v36's H2 label for item #1 without independent reasoning
  - Commit-message rooted-path discipline

INBOX_EOF

log "phase-1 INBOX seeded (v37 micro-soak: N=5 fan-out temporal classification)"
...
cat > "$WORKTREE/mind/INBOX.md" <<'INBOX_EOF'
# Inbox — Soak v37 phase 2 (commit-only, engines on)

Phase 1's five-item classification is in
`mind/research/v37-locomo-temporal-5-item-classification.md` under
...
CHARTER (v4.112 charter extraction will pass this to the witness panel):
  ...
  4. NO code modifications. NO ADR edits. NO test additions.
  5. NO modification of any other research note.
  6. Commit message: `[agent]` prefix + "classify items <id1>..<id5> as
     <labels>" (e.g. `[agent] classify items conv-26::qa14..conv-30::qa07 as H2,H1,H2,H3,H1`).
...
- [ ] Confirm five `## Item N:` paragraphs are present.
- [ ] Stage ONLY `mind/research/v37-locomo-temporal-5-item-classification.md`.
...
OVERSHOOT TRAPS the panel should reject:
  ...
  - **Classifying more or fewer than 5 items** — exactly 5.
  - **Proposing adapter or retrieval code changes** — R1 only.
...
This is the v37 micro-soak. Single research-note commit; nothing more.
...
log "─────────────────────────────────────────────────────────────"
log "long_cycle_soak_v37.sh complete"
...
log "        cat mind/research/v37-locomo-temporal-5-item-classification.md"
