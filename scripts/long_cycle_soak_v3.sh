#!/usr/bin/env bash
# scripts/long_cycle_soak_v3.sh — focused remediation retry
#
...
soak_refuse_concurrent "long_cycle_soak_v3.sh" || exit $?
...
BRANCH="chimera-soak/v3-$STAMP"
WORKTREE="${WORKTREE:-$REPO_ROOT/../chimera-soak-v3-$STAMP}"
...
LOG="$REPO_ROOT/state/long_cycle_v3_2026-05-20.log"
...
# ── pre-flight ─────────────────────────────────────────────────
log "─────────────────────────────────────────────────────────────"
log "long_cycle_soak_v3.sh start"
log "  branch         = $BRANCH"
...
git config --worktree remote.origin.pushurl \
    "no-push://disabled-for-soak-v3-$STAMP" 2>&1 | tee -a "$LOG" || true
...
# Inbox — Soak v3 phase 1 (investigation only, engines off)

The 2026-05-20 soak v1 surfaced one hot signature:

  ×2  tiers=haiku/sonnet  cycles=41→42  last=degenerate_loop_abort
      "Create a glossary of all capability terms used in the matrix
       as a standalone markdown file in mind/research"

ACT's `detect_degenerate_loop` (in `chimera/tools/loop_guard.py`)
fired twice on this task. Investigate whether the verdict was correct
or a false positive. Write the verdict to
`mind/research/loop-abort-investigation.md`.

## Phase 1 tasks

- [ ] Read `chimera/tools/loop_guard.py` in full and summarise the
  exact heuristic for `detect_degenerate_loop` — what window, what
  threshold, what equality test. Be precise; the verdict depends on
  it.

- [ ] Read the call site in `chimera/core/act.py` around the
  `detect_degenerate_loop(history)` invocation. Note what's in
  `history` at the point of the call (tool name + args, or full
  tool-call objects?). Note what happens when the verdict is True.

- [ ] Form a verdict on the soak-v1 case: false-positive / correct
  / unresolved. Defend it with a concrete reading of how the
  glossary subtask would have looked in `history` at the point
  `detect_degenerate_loop` fired. If you can't tell from the code
  alone, say so — that's the "unresolved" branch.
...
- [ ] Write all of the above to
  ...
  Under that heading: (a) the verdict, one sentence; (b) a one-
  paragraph fix sketch naming the function and the change; (c) a
  pseudocode test that would distinguish the new behaviour from
  the old.
...
log "─────────────────────────────────────────────────────────────"
log "long_cycle_soak_v3.sh complete"
