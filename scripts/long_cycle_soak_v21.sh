#!/usr/bin/env bash
# scripts/long_cycle_soak_v21.sh — focused remediation retry
#
...
soak_refuse_concurrent "long_cycle_soak_v21.sh" || exit $?
...
BRANCH="chimera-soak/v21-$STAMP"
WORKTREE="${WORKTREE:-$REPO_ROOT/../chimera-soak-v21-$STAMP}"
...
LOG="$REPO_ROOT/state/long_cycle_v21_${STAMP}.log"
...
# ── pre-flight ─────────────────────────────────────────────────
log "─────────────────────────────────────────────────────────────"
log "long_cycle_soak_v21.sh start"
log "  branch         = $BRANCH"
...
git config --worktree remote.origin.pushurl \
    "no-push://disabled-for-soak-v21-$STAMP" 2>&1 | tee -a "$LOG" || true
...
# Phase-1 INBOX — v21 target is `_check_uv_installed` doctor check.
...
# Inbox — Soak v21 phase 1 (investigation only, engines off)
...
log "phase-1 INBOX seeded (4 tasks, v21 uv-check target — proven add-one-function shape)"
...
# exit. Lives in scripts/soak_lib.sh so v21+ can share it.
...
cat > "$WORKTREE/mind/INBOX.md" <<'INBOX_EOF'
# Inbox — Soak v21 phase 2 (remediation, engines on)
...
This is the v21 minimal-shape attempt after v20 failed 5 times
...
log "─────────────────────────────────────────────────────────────"
log "long_cycle_soak_v21.sh complete"
