#!/usr/bin/env bash
# scripts/long_cycle_soak_v23.sh — focused remediation retry
#
...
soak_refuse_concurrent "long_cycle_soak_v23.sh" || exit $?
...
BRANCH="chimera-soak/v23-$STAMP"
WORKTREE="${WORKTREE:-$REPO_ROOT/../chimera-soak-v23-$STAMP}"
...
LOG="$REPO_ROOT/state/long_cycle_v23_${STAMP}.log"
...
# ── pre-flight ─────────────────────────────────────────────────
log "─────────────────────────────────────────────────────────────"
log "long_cycle_soak_v23.sh start"
log "  branch         = $BRANCH"
...
git config --worktree remote.origin.pushurl \
    "no-push://disabled-for-soak-v23-$STAMP" 2>&1 | tee -a "$LOG" || true
...
# Phase-1 INBOX — v23 target is wiring v4.116's check_charter_file_count
...
# Inbox — Soak v23 phase 1 (investigation only, engines off)
...
log "phase-1 INBOX seeded (8 tasks, v23 v4.116-wiring target — multi-file under load-bearing chain)"
...
# exit. Lives in scripts/soak_lib.sh so v23+ can share it.
...
cat > "$WORKTREE/mind/INBOX.md" <<'INBOX_EOF'
# Inbox — Soak v23 phase 2 (remediation, engines on)
...
This is the v23 multi-file wire-up under the load-bearing chain.
...
log "─────────────────────────────────────────────────────────────"
log "long_cycle_soak_v23.sh complete"
