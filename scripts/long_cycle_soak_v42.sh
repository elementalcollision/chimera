#!/usr/bin/env bash
# scripts/long_cycle_soak_v42.sh — R3 build-capability, ladder rung 3
# (multi-file isolated build: two new files + an authored import boundary, N=1).
#
# Charter: mind/research/v42-boxtable-design.md. The v40 capstone
...
# module pair chimera/boxtable*.py that nothing imports at load time, tested
...
#
# Target: create TWO new files with an import boundary —
#   chimera/boxtable_cells.py (col_widths, pad_cell) and
#   chimera/boxtable.py (format_table, which imports those helpers)
# so the pre-written test tests/test_boxtable.py (already on main, 6 tests,
...
#   phase 1 (engines OFF, no commits): the agent CREATES chimera/boxtable.py + boxtable_cells.py,
...
#   phase 2 (engines ON, commit-only): commit both boxtable files + the
#     postmortem. The ADR 0146 pre-commit scope check (allowlist =
#     {chimera/boxtable.py, chimera/boxtable_cells.py}) refuses any other code path, INCLUDING an
...
#
# Cost: hard cap $3.00 total. v42 runs on the hardened substrate (B1/B2 +
# postmortem sub-chips); v40′/v41 already cleared rungs 1–2.
#
...
soak_refuse_concurrent "long_cycle_soak_v42.sh" || exit $?
...
BRANCH="chimera-soak/v42-boxtable-$STAMP"
WORKTREE="${WORKTREE:-$REPO_ROOT/../chimera-soak-v42-$STAMP}"
...
export CHIMERA_SOAK_RUN_ID="v42-boxtable-$STAMP"
...
GATE_TEST_CMD="uv run --extra dev pytest -q tests/test_boxtable.py"

READY_MARKER="## READY-FOR-REMEDIATION"
LOG="$REPO_ROOT/state/long_cycle_v42_${STAMP}.log"
...
log "─────────────────────────────────────────────────────────────"
log "long_cycle_soak_v42.sh — FIRST R3 build-capability soak (N=1)"
log "  branch         = $BRANCH"
...
git config --worktree remote.origin.pushurl \
    "no-push://disabled-for-soak-v42-$STAMP" 2>&1 | tee -a "$LOG" || true
...
DELIVERABLE_REL="mind/research/v42-boxtable-postmortem.md"
...
# Inbox — Soak v42 phase 1 (BUILD; engines off, no commits yet)

**Chip**: create TWO new files with an import boundary you author:
\`chimera/boxtable_cells.py\` (helpers \`col_widths\`, \`pad_cell\`) and
\`chimera/boxtable.py\` (\`format_table\`, which imports those helpers),
so the pre-written test passes. You are writing code, not classifying.
**Atomic op class**: code + postmortem. **Code goes in TWO NEW files:
\`chimera/boxtable.py\` and \`chimera/boxtable_cells.py\`. Do NOT touch
\`chimera/cli.py\` or any other existing source.**

## The contract lives in the test (read it; do NOT edit it)

The pre-written test \`tests/test_boxtable.py\` is on main and defines
...
["uv","run","--extra","dev","pytest","-q","tests/test_boxtable.py"]:
...
Notes:
  ...
  - Pre-implementation you will see 6 failed. When your implementation
    is correct you will see 6 passed (NOT "6 skipped" — skipped means
...
RUN the test and SEEN \`6 passed\`. Reading and implementing are steps
WITHIN it, not separate tasks.

- [ ] **Build both boxtable files and prove green.** In ONE task:
  (a) read \`tests/test_boxtable.py\` to learn the EXACT contract — it
  imports \`from chimera.boxtable import format_table\` AND
  \`from chimera.boxtable_cells import col_widths, pad_cell\`;
  (b) CREATE \`chimera/boxtable_cells.py\` with the helpers, then CREATE
  \`chimera/boxtable.py\` with \`format_table\` importing them
  (\`from chimera.boxtable_cells import col_widths, pad_cell\`);
  (c) run \`$GATE_TEST_CMD\` via the shell tool
  (argv ["uv","run","--extra","dev","pytest","-q","tests/test_boxtable.py"]);
  (d) read the failures and EDIT the files, then RUN THE TEST AGAIN;
  repeat (b)–(d) until the output literally contains \`6 passed\`.
  **You are NOT done with this task until you have run that command and
  its output shows \`6 passed\`. Do not mark it complete on the strength
...
- [ ] **Write the postmortem** \`$DELIVERABLE_REL\` (ONLY after the test
  shows 6 passed) using \`mind/postmortems/TEMPLATE-soak-postmortem.md\`.
...
The test asserts EXACT strings. \`col_widths(rows)\` is the per-column max
cell length over all rows (\`[]\` for \`[]\`); \`pad_cell(text, width)\` is
\`text.ljust(width)\` (returned UNCHANGED if longer than width — no
truncation). \`format_table(rows)\`: EMPTY rows → \`""\`; otherwise render
each row as \`" | ".join(pad_cell(cell, w) for cell, w in zip(row, widths))\`
using \`col_widths\`, join rows with \`"\\n"\`, trailing \`"\\n"\`. No separator
line. Example: \`format_table([["a","bb"],["ccc","d"]])\` → \`"a   | bb\\nccc | d \\n"\`.
\`boxtable.py\` MUST import the helpers from \`boxtable_cells.py\`. Read the
test's exact expected strings — get them right, do not approximate.

## Two specific traps (these failed prior attempts — avoid them)

  - **Keep all imports at module top-level** in BOTH files (the cross-file
    \`from chimera.boxtable_cells import ...\` goes at the top of
    \`boxtable.py\`). Do NOT put an \`import\` inside a function — a
    function-local import shadows the module name across the whole function
    and raises UnboundLocalError. (This os/Path-shadow class broke prior
    attempts; a pre-commit gate now refuses it.)
...
SCOPE (locked): create/edit ONLY \`chimera/boxtable.py\` and
\`chimera/boxtable_cells.py\` for code; write ONLY the postmortem under
mind/research/. Do NOT edit the test, do NOT touch \`chimera/cli.py\` or any
other existing source, any ADR, or pyproject.toml. No CLI wiring — this
charter is the two standalone modules only.

OVERSHOOT TRAPS the panel should reject:
  - Editing \`tests/test_boxtable.py\` (read-only; refused at commit)
  - Touching \`chimera/cli.py\` or any code file other than the two
    \`chimera/boxtable*.py\` files (refused at commit)
...
log "phase-1 INBOX seeded (v42 R3 build: create chimera/boxtable.py + boxtable_cells.py)"
...
# the boxtable files make the 6 tests pass AND the postmortem is present
...
Phase 1 created \`chimera/boxtable.py\` + \`chimera/boxtable_cells.py\` and
the gated test passes. Phase 2 commits both files and the postmortem.

CHARTER:
  1. SCOPE: \`chimera/boxtable.py\` + \`chimera/boxtable_cells.py\` (the
     implementation) + \`$DELIVERABLE_REL\` (the postmortem). NOTHING else.
  2. The pre-commit scope check (ADR 0146) is bound to this charter's
     design note; its allowlist is {chimera/boxtable.py,
     chimera/boxtable_cells.py}. Any other code path — INCLUDING an edit to
     tests/test_boxtable.py or to chimera/cli.py — is REFUSED.
  3. Re-run \`$GATE_TEST_CMD\` first; confirm 6 passed before committing.
  4. Commit message: \`[agent]\` prefix + "create chimera/boxtable".
     Do NOT cite paths absent from the diff (ADR 0122).

## Phase 2 tasks
- [ ] Re-run the gated test; confirm 6 passed.
- [ ] Stage ONLY \`chimera/boxtable.py\`, \`chimera/boxtable_cells.py\`, and
  \`$DELIVERABLE_REL\`.
...
OVERSHOOT TRAPS the panel should reject:
  ...
  - Committing any code file other than the two chimera/boxtable*.py files
...
SOFT_SENTINEL_ALLOWED_FILES="chimera/boxtable.py chimera/boxtable_cells.py $DELIVERABLE_REL"
SOFT_SENTINEL_TEST_CMD="$GATE_TEST_CMD"
...
log "─────────────────────────────────────────────────────────────"
log "long_cycle_soak_v42.sh complete"
...
log "        CHIMERA_V40_GATE=1 uv run --extra dev pytest -q tests/test_boxtable.py"
log "        git diff main..HEAD --name-only   # expect: chimera/boxtable.py + chimera/boxtable_cells.py + $DELIVERABLE_REL"
