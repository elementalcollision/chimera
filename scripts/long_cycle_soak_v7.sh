#!/usr/bin/env bash
# scripts/long_cycle_soak_v7.sh — focused remediation retry
#
...
soak_refuse_concurrent "long_cycle_soak_v7.sh" || exit $?
...
BRANCH="chimera-soak/v7-$STAMP"
WORKTREE="${WORKTREE:-$REPO_ROOT/../chimera-soak-v7-$STAMP}"
...
LOG="$REPO_ROOT/state/long_cycle_v7_${STAMP}.log"
...
# ── pre-flight ─────────────────────────────────────────────────
log "─────────────────────────────────────────────────────────────"
log "long_cycle_soak_v7.sh start"
log "  branch         = $BRANCH"
...
git config --worktree remote.origin.pushurl \
    "no-push://disabled-for-soak-v7-$STAMP" 2>&1 | tee -a "$LOG" || true
...
# Inbox — Soak v7 phase 1 (investigation only, engines off)
...
log "─────────────────────────────────────────────────────────────"
log "long_cycle_soak_v7.sh complete"
