...
# Phase-1 INBOX — v26 target is sub-soak A in the v4.116 wiring
...
# Inbox — Soak v26 phase 1 (investigation only, engines off)

**Sub-soak B** of the v4.116 wiring decomposition
(docs/wiring-decomposition-methodology.md).
**Atomic op**: `add-call-site`.
**Target**: Add a call site that invokes check_charter_file_count and populates ActResult.charter_file_count_violations (the field shipped in v25).

The template: **v4.115's call site at chimera/core/act.py:2033 (check_commit_message_diff_drift)**. Copy that shape with the
...
- [ ] Read `chimera/core/act.py` to confirm the integration point hasn't
  drifted.
...
- [ ] Spec the addition. Write to `mind/research/v26-call-site-design.md`.
...
log "phase-1 INBOX seeded (4 tasks, v26 v4.116-wiring sub-soak A: ActResult field)"
...
# exit. Lives in scripts/soak_lib.sh so v26+ can share it.
...
log "        cat mind/research/v26-actresult-field-design.md"
log "        cat mind/research/v26-actresult-field-remediation.md"
