...
# Phase-1 INBOX — v27 target is sub-soak A in the v4.116 wiring
...
# Inbox — Soak v27 phase 1 (investigation only, engines off)

**Sub-soak C** of the v4.116 wiring decomposition
(docs/wiring-decomposition-methodology.md).
**Atomic op**: `add-line`.
**Target**: Add 'charter_file_count' to ESCALATING_FINISH_REASONS in chimera/core/escalation.py so the three-strikes auto-skip path applies. SHIPS LAST per Q1 (chain activator).

The template: **v4.115's entry at chimera/core/escalation.py:147**. Copy that shape with the
...
- [ ] Read `chimera/core/escalation.py` to confirm the integration point hasn't
  drifted.
...
- [ ] Spec the addition. Write to `mind/research/v27-escalation-entry-design.md`.
...
log "phase-1 INBOX seeded (4 tasks, v27 v4.116-wiring sub-soak A: ActResult field)"
...
# exit. Lives in scripts/soak_lib.sh so v27+ can share it.
...
SOFT_SENTINEL_ALLOWED_FILES="chimera/core/escalation.py tests/test_charter_file_count.py"
...
log "        cat mind/research/v27-actresult-field-design.md"
log "        cat mind/research/v27-actresult-field-remediation.md"
