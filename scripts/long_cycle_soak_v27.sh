#!/usr/bin/env bash
# scripts/long_cycle_soak_v27.sh — focused remediation retry
#
...
soak_refuse_concurrent "long_cycle_soak_v27.sh" || exit $?
...
BRANCH="chimera-soak/v27-$STAMP"
WORKTREE="${WORKTREE:-$REPO_ROOT/../chimera-soak-v27-$STAMP}"
...
LOG="$REPO_ROOT/state/long_cycle_v27_${STAMP}.log"
...
# ── pre-flight ─────────────────────────────────────────────────
log "─────────────────────────────────────────────────────────────"
log "long_cycle_soak_v27.sh start"
log "  branch         = $BRANCH"
...
git config --worktree remote.origin.pushurl \
    "no-push://disabled-for-soak-v27-$STAMP" 2>&1 | tee -a "$LOG" || true
...
# Phase-1 INBOX — v27 target is v4.116 wiring sub-soak C: escalation entry.
...
panel from this task text):
  ...
  1. ONE new line in `chimera/core/escalation.py`: add `"charter_file_count"` to the ESCALATING_FINISH_REASONS set/list. Place it alongside the existing v4.115 entry (~line 147).
  2. SEMANTICS: presence in the set means the three-strikes auto-skip path applies when this finish_reason fires three times in a row on the same task.
  3. PATTERN: same single-string entry shape as v4.115.
  4. NO modification of existing entries or set/list structure (charter #4).
  5. NO call site changes in act.py (v26's job) or other wiring.
  6. NO new helper functions — this is a single string addition.
  7. The set membership must NEVER raise on lookup. Standard Python set.
...
- [ ] ONE new line: `"charter_file_count",` in the ESCALATING_FINISH_REASONS set, placed alongside the existing v4.115 entry.

- [ ] Add ONE new test asserting `"charter_file_count" in ESCALATING_FINISH_REASONS`.
...
- [ ] Re-run the targeted test post-commit and write the summary
  line into `mind/research/v27-escalation-entry-remediation.md` under `## Test
...
OVERSHOOT TRAPS the panel should reject:
  ...
  - Touching act.py's call site (v26's job, already shipped)
...
log "─────────────────────────────────────────────────────────────"
log "long_cycle_soak_v27.sh complete"
...
log "        cat mind/research/v27-escalation-entry-design.md"
log "        cat mind/research/v27-escalation-entry-remediation.md"
