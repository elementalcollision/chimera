#!/usr/bin/env bash
# scripts/long_cycle_soak_v12.sh — focused remediation retry
#
...
soak_refuse_concurrent "long_cycle_soak_v12.sh" || exit $?
...
BRANCH="chimera-soak/v12-$STAMP"
WORKTREE="${WORKTREE:-$REPO_ROOT/../chimera-soak-v12-$STAMP}"
...
LOG="$REPO_ROOT/state/long_cycle_v12_${STAMP}.log"
...
# ── pre-flight ─────────────────────────────────────────────────
log "─────────────────────────────────────────────────────────────"
log "long_cycle_soak_v12.sh start"
log "  branch         = $BRANCH"
...
git config --worktree remote.origin.pushurl \
    "no-push://disabled-for-soak-v12-$STAMP" 2>&1 | tee -a "$LOG" || true
...
# Inbox — Soak v12 phase 1 (investigation only, engines off)
...
log "─────────────────────────────────────────────────────────────"
log "long_cycle_soak_v12.sh complete"
