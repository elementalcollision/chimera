#!/usr/bin/env bash
# scripts/long_cycle_soak_v13.sh — focused remediation retry
#
...
soak_refuse_concurrent "long_cycle_soak_v13.sh" || exit $?
...
BRANCH="chimera-soak/v13-$STAMP"
WORKTREE="${WORKTREE:-$REPO_ROOT/../chimera-soak-v13-$STAMP}"
...
LOG="$REPO_ROOT/state/long_cycle_v13_${STAMP}.log"
...
# ── pre-flight ─────────────────────────────────────────────────
log "─────────────────────────────────────────────────────────────"
log "long_cycle_soak_v13.sh start"
log "  branch         = $BRANCH"
...
git config --worktree remote.origin.pushurl \
    "no-push://disabled-for-soak-v13-$STAMP" 2>&1 | tee -a "$LOG" || true
...
log "─────────────────────────────────────────────────────────────"
log "long_cycle_soak_v13.sh complete"
