#!/usr/bin/env bash
# scripts/long_cycle_soak_v40prime.sh — R3 build-capability, ISOLATED-target
# re-charter of v40 (N=1).
#
# Charter: mind/research/v40prime-mindcount-design.md. The v40 capstone
...
# module chimera/mindcount.py that nothing imports at load time, tested
...
#
# Target: create chimera/mindcount.py exposing
#   format_mind_counts(mind_dir) -> str
# so the pre-written test tests/test_mindcount.py (already on main, 5 tests,
...
#   phase 1 (engines OFF, no commits): the agent CREATES chimera/mindcount.py,
...
#   phase 2 (engines ON, commit-only): commit chimera/mindcount.py + the
#     postmortem. The ADR 0146 pre-commit scope check (allowlist =
#     {chimera/mindcount.py}) refuses any other code path, INCLUDING an
...
#
# Cost: hard cap $3.00 total. This is v40′ attempt #1; the harness is
# defect-free on all v40 fronts (PRs #141/#142/#143).
#
...
soak_refuse_concurrent "long_cycle_soak_v40prime.sh" || exit $?
...
BRANCH="chimera-soak/v40prime-mindcount-$STAMP"
WORKTREE="${WORKTREE:-$REPO_ROOT/../chimera-soak-v40prime-$STAMP}"
...
export CHIMERA_SOAK_RUN_ID="v40prime-mindcount-$STAMP"
...
LOG="$REPO_ROOT/state/long_cycle_v40prime_${STAMP}.log"
...
log "─────────────────────────────────────────────────────────────"
log "long_cycle_soak_v40prime.sh — FIRST R3 build-capability soak (N=1)"
log "  branch         = $BRANCH"
...
git config --worktree remote.origin.pushurl \
    "no-push://disabled-for-soak-v40prime-$STAMP" 2>&1 | tee -a "$LOG" || true
...
DELIVERABLE_REL="mind/research/v40prime-mindcount-postmortem.md"
...
**Chip**: create a NEW module \`chimera/mindcount.py\` exposing
\`format_mind_counts(mind_dir) -> str\` so the pre-written test passes.
You are writing code, not classifying.
**Atomic op class**: code + postmortem. **Code goes in ONE NEW file:
\`chimera/mindcount.py\`. Do NOT touch \`chimera/cli.py\` or any other
existing source.**

## The contract lives in the test (read it; do NOT edit it)

The pre-written test \`tests/test_mindcount.py\` is on main and defines
...
["uv","run","--extra","dev","pytest","-q","tests/test_mindcount.py"]:
...
- [ ] **Build \`chimera/mindcount.py\` and prove it green.** In ONE task:
  (a) read \`tests/test_mindcount.py\` to learn the EXACT contract and the
  function signature it imports (\`from chimera.mindcount import
  format_mind_counts\`); (b) CREATE \`chimera/mindcount.py\` with that
  function; (c) run \`$GATE_TEST_CMD\` via the shell tool
  (argv ["uv","run","--extra","dev","pytest","-q","tests/test_mindcount.py"]);
  (d) read the failures and EDIT \`chimera/mindcount.py\`, then RUN THE TEST
  AGAIN; repeat (b)–(d) until the output literally contains \`5 passed\`.
...
The test asserts the EXACT return string. \`format_mind_counts(mind_dir)\`
returns **one line per top-level entry** under mind_dir, formatted
\`"<name>: <count>\\n"\`, sorted alphabetically, where a directory's count
is the RECURSIVE number of files beneath it (any depth), a top-level file
is \`1\`, and hidden entries (names starting with \`.\`) are skipped. It is
a PURE function — return the string, do NOT print. It is NOT a single
summary line like "N files, M dirs" — that will fail every test.

## Two specific traps (these failed prior attempts — avoid them)

  - **Keep all imports at module top-level** in \`chimera/mindcount.py\`
    (\`import os\`, \`from pathlib import Path\`). Do NOT put an \`import\`
    inside a function — a function-local import shadows the module name
    across the whole function and raises UnboundLocalError. (This class of
    bug — os/Path shadowing — broke prior attempts.)
...
SCOPE (locked): create/edit ONLY \`chimera/mindcount.py\` for code; write
...
OVERSHOOT TRAPS the panel should reject:
  - Editing \`tests/test_mindcount.py\` (read-only; refused at commit)
  - Touching \`chimera/cli.py\` or any code file other than
    \`chimera/mindcount.py\` (refused at commit)
  - Claiming the tests pass without running the gated command
    (the test-run ledger records ground truth)
  - Returning/printing a summary line instead of the per-entry contract
...
log "phase-1 INBOX seeded (v40prime R3 build: create chimera/mindcount.py)"
...
# chimera/mindcount.py makes the 5 tests pass AND the postmortem is present
...
CHARTER:
  ...
  2. The pre-commit scope check (ADR 0146) is bound to this charter's
     ...
     — INCLUDING an edit to tests/test_mindcount.py — is REFUSED.
...
SOFT_SENTINEL_ALLOWED_FILES="chimera/mindcount.py $DELIVERABLE_REL"
SOFT_SENTINEL_TEST_CMD="$GATE_TEST_CMD"
...
log "─────────────────────────────────────────────────────────────"
log "long_cycle_soak_v40prime.sh complete"
...
log "        CHIMERA_V40_GATE=1 uv run --extra dev pytest -q tests/test_mindcount.py"
log "        git diff main..HEAD --name-only   # expect: chimera/mindcount.py + $DELIVERABLE_REL"
