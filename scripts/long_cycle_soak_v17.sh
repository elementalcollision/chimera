#!/usr/bin/env bash
# scripts/long_cycle_soak_v17.sh — focused remediation retry
#
...
soak_refuse_concurrent "long_cycle_soak_v17.sh" || exit $?
...
BRANCH="chimera-soak/v17-$STAMP"
WORKTREE="${WORKTREE:-$REPO_ROOT/../chimera-soak-v17-$STAMP}"
...
LOG="$REPO_ROOT/state/long_cycle_v17_${STAMP}.log"
...
# ── pre-flight ─────────────────────────────────────────────────
log "─────────────────────────────────────────────────────────────"
log "long_cycle_soak_v17.sh start"
log "  branch         = $BRANCH"
...
git config --worktree remote.origin.pushurl \
    "no-push://disabled-for-soak-v17-$STAMP" 2>&1 | tee -a "$LOG" || true
...
# Inbox — Soak v17 phase 1 (investigation only, engines off)
...
log "─────────────────────────────────────────────────────────────"
log "long_cycle_soak_v17.sh complete"
