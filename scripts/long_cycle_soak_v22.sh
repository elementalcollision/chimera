#!/usr/bin/env bash
# scripts/long_cycle_soak_v22.sh — focused remediation retry
#
...
soak_refuse_concurrent "long_cycle_soak_v22.sh" || exit $?
...
BRANCH="chimera-soak/v22-$STAMP"
WORKTREE="${WORKTREE:-$REPO_ROOT/../chimera-soak-v22-$STAMP}"
...
LOG="$REPO_ROOT/state/long_cycle_v22_${STAMP}.log"
...
# ── pre-flight ─────────────────────────────────────────────────
log "─────────────────────────────────────────────────────────────"
log "long_cycle_soak_v22.sh start"
log "  branch         = $BRANCH"
...
git config --worktree remote.origin.pushurl \
    "no-push://disabled-for-soak-v22-$STAMP" 2>&1 | tee -a "$LOG" || true
...
# Phase-1 INBOX — v22 target is wiring v4.116's check_charter_file_count
...
# Inbox — Soak v22 phase 1 (investigation only, engines off)
...
log "phase-1 INBOX seeded (8 tasks, v22 v4.116-wiring target — multi-file under load-bearing chain)"
...
# exit. Lives in scripts/soak_lib.sh so v22+ can share it.
...
cat > "$WORKTREE/mind/INBOX.md" <<'INBOX_EOF'
# Inbox — Soak v22 phase 2 (remediation, engines on)
...
- [ ] Run the targeted test file: `uv run pytest
  ...
  a new file) and write the summary line into
  `mind/research/charter-file-count-wiring-remediation.md`
  under `## Test results`.
...
This is the v22 multi-file wire-up under the load-bearing chain.
...
SOFT_SENTINEL_TEST_CMD="uv run pytest tests/test_charter_file_count.py tests/test_charter_file_count_wiring.py -q 2>&1 | tail -1 | grep -E 'passed|no tests ran'"
...
log "─────────────────────────────────────────────────────────────"
log "long_cycle_soak_v22.sh complete"
