...
# ADR 0163 self-PR is now centralized in real_task_soak.sh (the runner this
# wrapper invokes via `eval`), so it fires THERE during the build — no hook here
# (avoids a double-fire: the inner runner and this collector both attempting it).
