...
# 2. Enforcement ON, genuine self-commit only.
...
# 3. Build it: run the self-selected soak invocation under enforcement.
log "── launching self-determined soak (enforced) ──"
...
if [ -n "$wt" ] && [ -d "$wt" ]; then
    ...
    sha="$(cd "$wt" && git log --format='%h %s' main..HEAD 2>/dev/null | grep '\[agent\]' | head -1)"
    [ -n "$sha" ] && committed="yes ($sha)"
    touched="$(cd "$wt" && git diff --name-only main..HEAD 2>/dev/null | tr '\n' ' ')"
...
log "  review        : cd $wt && git log -p main..HEAD && cat state/critic-gate-log.jsonl"
echo
