#!/usr/bin/env bash
# scripts/long_cycle_soak_v45.sh — R3 build, SECOND real feature.
#
# Charter: mind/research/v45-soakbreakdown-design.md. The SECOND real feature
# (v44 was the first). Also validates the three build-soak fixes landed today:
# #168 over-claim-only (no commit deadlock), #173 artifact-detail ledger (churn
# diagnosable), #174 witness asymmetric voting (witness_rejected near-zero on a
# correct diff). Built with the SAME falsification discipline, contract-pinned
# by a pre-written test.
#
# Value: a finish-reason COUNT table (the `3 artifact_missing / 8
# skipped_three_strikes / 4 stop` summary the operator computes by hand with
# `jq | sort | uniq -c`). The aggregate companion to v44's soak_summary
# (per-cycle table). This soak lands the isolated module; the CLI verb
# (`chimera soak breakdown`) is a separate operator chip.
#
# Target: create ONE new standalone module chimera/soak_breakdown.py
#   - format_finish_reason_breakdown(act_rows) -> str  (pure, test-pinned core)
#   - format_soak_breakdown(mind_dir, run_id) -> str   (reads act-tools.jsonl)
# so the pre-written test tests/test_soak_breakdown.py (on main, gated, failing)
...
#   phase 2 (engines ON, commit-only): commit chimera/soak_breakdown.py + the
#     postmortem. The ADR 0146 scope check (allowlist={chimera/soak_breakdown.py})
...
soak_refuse_concurrent "long_cycle_soak_v45.sh" || exit $?
...
BRANCH="chimera-soak/v45-soakbreakdown-$STAMP"
WORKTREE="${WORKTREE:-$REPO_ROOT/../chimera-soak-v45-soakbreakdown-$STAMP}"
...
export CHIMERA_SOAK_RUN_ID="v45-soakbreakdown-$STAMP"
export CHIMERA_ACT_BUDGET_SECONDS="${CHIMERA_ACT_BUDGET_SECONDS:-600}"

GATE_TEST_CMD="uv run --extra dev pytest -q tests/test_soak_breakdown.py"

READY_MARKER="## READY-FOR-REMEDIATION"
LOG="$REPO_ROOT/state/long_cycle_v45_${STAMP}.log"
...
log "─────────────────────────────────────────────────────────────"
log "long_cycle_soak_v45.sh — R3 build, SECOND real feature"
log "  branch         = $BRANCH"
...
git config --worktree remote.origin.pushurl \
    "no-push://disabled-for-soak-v45-soakbreakdown-$STAMP" 2>&1 | tee -a "$LOG" || true
...
DELIVERABLE_REL="mind/research/v45-soakbreakdown-postmortem.md"
...
# Inbox — Soak v45-soakbreakdown phase 1 (BUILD; engines off, no commits yet)

**Chip**: create ONE new module \`chimera/soak_breakdown.py\` so the pre-written
...
\`chimera/soak_breakdown.py\`. Do NOT touch \`chimera/cli.py\`,
...
The pre-written test \`tests/test_soak_breakdown.py\` is on main and defines the
...
["uv","run","--extra","dev","pytest","-q","tests/test_soak_breakdown.py"]:
...
Notes:
  ...
  - Pre-implementation: 6 failed. Correct implementation: 6 passed (NOT "6
    skipped").

## Phase 1 tasks — exactly TWO; do them in order

This is a TEST-DRIVEN build. The first task is NOT done until you have RUN the
test and SEEN \`6 passed\`.

- [ ] **Build \`chimera/soak_breakdown.py\` and prove green.** In ONE task: read
  \`tests/test_soak_breakdown.py\` for the EXACT contract — two functions:
  \`format_finish_reason_breakdown(act_rows)\` (a pure function counting each
  record's \`finish_reason\` and returning a markdown table sorted by COUNT
  DESCENDING then reason ASCENDING, with a \`**Σ**\` total row; empty input →
  \`""\`) and \`format_soak_breakdown(mind_dir, run_id)\` (reads
  \`<mind_dir>/soak/<run_id>/act-tools.jsonl\` via
  \`chimera.core.soak_ledger._read_jsonl\`, returns a
  \`# Soak <run_id> — finish-reason breakdown (<N> cycles)\` headline + the
  table, or \`# Soak <run_id> — no ACT records\\n\` when empty). CREATE the
  module, run \`$GATE_TEST_CMD\`, read failures, EDIT, RUN AGAIN until the output
  literally contains \`6 passed\`. The test asserts EXACT strings — match the
  header \`| finish_reason | count |\`, the \`|---|---:|\` separator, the
  sort order, and the trailing newline precisely; do not approximate.
  **You are NOT done until that command's output shows \`6 passed\`.**
- [ ] **Write the postmortem** \`$DELIVERABLE_REL\` (ONLY after 6 passed) using
...
SCOPE (locked): create/edit ONLY \`chimera/soak_breakdown.py\` for code; write ONLY
...
OVERSHOOT TRAPS the panel should reject:
  - Editing \`tests/test_soak_breakdown.py\` (read-only; refused at commit)
  - Modifying \`chimera/core/soak_ledger.py\` or any file other than
    \`chimera/soak_breakdown.py\`
...
log "phase-1 INBOX seeded (v45-soakbreakdown R3 build: chimera/soak_breakdown.py)"
...
cat > "$WORKTREE/mind/INBOX.md" <<INBOX_EOF
# Inbox — Soak v45-soakbreakdown phase 2 (commit-only, engines on)

Phase 1 created \`chimera/soak_breakdown.py\` and the gated test passes. Phase 2
commits the module and the postmortem.

CHARTER:
  1. SCOPE: \`chimera/soak_breakdown.py\` (the implementation) + \`$DELIVERABLE_REL\`
     (the postmortem). NOTHING else.
  2. The pre-commit scope check (ADR 0146) allowlist is {chimera/soak_breakdown.py}.
     Any other code path — INCLUDING tests/test_soak_breakdown.py,
  ...
  4. ONE commit. The message MUST begin with the literal token \`[agent]\` as its
     FIRST characters — e.g. \`[agent] create chimera/soak_breakdown.py\`. This is
...
- [ ] Stage ONLY \`chimera/soak_breakdown.py\` and \`$DELIVERABLE_REL\`.
- [ ] Commit with a message starting with \`[agent]\` (e.g. \`[agent] create chimera/soak_breakdown.py\`).
...
OVERSHOOT TRAPS the panel should reject:
  ...
  - Committing any code file other than chimera/soak_breakdown.py
...
SOFT_SENTINEL_ALLOWED_FILES="chimera/soak_breakdown.py $DELIVERABLE_REL"
SOFT_SENTINEL_TEST_CMD="$GATE_TEST_CMD"
...
log "─────────────────────────────────────────────────────────────"
log "long_cycle_soak_v45.sh complete"
...
log "        git diff main..HEAD --name-only   # expect: chimera/soak_breakdown.py + $DELIVERABLE_REL"
