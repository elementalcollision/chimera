...
# Phase-1 INBOX — v28 target is sub-soak A in the v4.116 wiring
...
**Sub-soak D** of the v4.116 wiring decomposition
(docs/wiring-decomposition-methodology.md).
**Atomic op**: `add-line`.
**Target**: Add 'charter_file_count': -1 to FINISH_REASON_TRUST_DELTAS in chimera/trust/manager.py. SHIPS LAST per Q1 (paired with v27 to activate v4.116).

The template: **v4.115's entry in FINISH_REASON_TRUST_DELTAS (grep for commit_message_diff_drift)**. Copy that shape with the
...
- [ ] Read `chimera/trust/manager.py` to confirm the integration point hasn't
  drifted.
...
- [ ] Spec the addition. Write to `mind/research/v28-trust-delta-design.md`.
...
log "phase-1 INBOX seeded (4 tasks, v28 v4.116-wiring sub-soak A: ActResult field)"
...
SOFT_SENTINEL_ALLOWED_FILES="chimera/trust/manager.py tests/test_charter_file_count.py"
...
log "        cat mind/research/v28-actresult-field-design.md"
log "        cat mind/research/v28-actresult-field-remediation.md"
