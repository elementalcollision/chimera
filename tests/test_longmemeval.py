...
# ── User-context heuristic (ADR 0138 redesigned, post-PR #73) ────


def _user_ctx_block(self_card: str) -> str | None:
    """Return the body of the ``## User context`` section, or None
    if the section is absent."""
    if "## User context" not in self_card:
        return None
    after = self_card.split("## User context", 1)[1]
    return after.split("## History", 1)[0]


def test_user_ctx_filler_am_im_my_not_extracted(adapter, tmp_path):
    """Regression guard for PR #73 spike finding: bare `I am` / `I'm` /
    `my <noun>` are conversational filler and MUST NOT surface."""
    item = LongMemEvalItem(
        item_id="ux-filler", question="?",
        history=[[
            {"role": "user", "content": "I am wondering about trains."},
            {"role": "user", "content": "I'm here to ask a question."},
            {"role": "user", "content": "my apologies for the delay."},
            {"role": "user", "content": "my thoughts on this are mixed."},
        ]],
    )
    adapter.ingest_history(item)
    self_card = (tmp_path / "mind" / "peers" / "self.md").read_text()
    # No User context section at all — none of these turns carry a real signal.
    assert "## User context" not in self_card


def test_user_ctx_negation_and_rejection_extracted(adapter, tmp_path):
    """Negation/rejection IS preference signal and must surface."""
    item = LongMemEvalItem(
        item_id="ux-neg", question="?",
        history=[[
            {"role": "user", "content": "I don't like true crime podcasts."},
            {"role": "user", "content": "Honestly, not interested in horror movies."},
            {"role": "user", "content": "I never enjoy crowded restaurants."},
        ]],
    )
    adapter.ingest_history(item)
    self_card = (tmp_path / "mind" / "peers" / "self.md").read_text()
    block = _user_ctx_block(self_card)
    assert block is not None
    assert "true crime" in block
    assert "horror" in block
    assert "crowded" in block


def test_user_ctx_recent_n_drops_stale_preferences(adapter, tmp_path):
    """A preference stated in turn 0 and rejected in turn 9 should
    surface only the recent state — the heuristic considers the last
    N=5 user turns, so the stale early statement is dropped."""
    turns = []
    # Turn 0: stale preference (should NOT surface).
    turns.append({"role": "user", "content": "I love spicy Thai food."})
    # Filler turns 1..8.
    for i in range(8):
        turns.append({"role": "user", "content": f"random question number {i}"})
    # Turn 9: recent rejection (should surface).
    turns.append({"role": "user", "content": "Actually I don't like spicy food anymore."})
    item = LongMemEvalItem(
        item_id="ux-recency", question="?",
        history=[turns],
    )
    adapter.ingest_history(item)
    self_card = (tmp_path / "mind" / "peers" / "self.md").read_text()
    block = _user_ctx_block(self_card)
    assert block is not None
    # The rejection survives because it's in the last-5 window AND matches negation.
    assert "don't like spicy" in block
    # The stale "I love spicy Thai food" is outside the last-5 window — must be gone.
    assert "Thai food" not in block


def test_user_ctx_identity_vs_filler(adapter, tmp_path):
    """`I'm a vegetarian` (identity) surfaces; `I'm wondering` (filler)
    doesn't. The identity regex requires `a`/`an` after `I'm` to avoid
    the broad-`I'm` false-positive class PR #73 surfaced."""
    item = LongMemEvalItem(
        item_id="ux-identity", question="?",
        history=[[
            {"role": "user", "content": "I'm a vegetarian."},
            {"role": "user", "content": "I'm wondering what to cook."},
        ]],
    )
    adapter.ingest_history(item)
    self_card = (tmp_path / "mind" / "peers" / "self.md").read_text()
    ...
    assert "wondering" not in block


def test_user_ctx_omitted_when_no_signal(adapter, tmp_path):
    """Defensive: items with no preference-bearing turns ingest cleanly
    with the section omitted — preserves pre-chip layout for those items."""
    item = LongMemEvalItem(
        item_id="ux-empty", question="?",
        history=[[
    ...
    self_card = (tmp_path / "mind" / "peers" / "self.md").read_text()
    assert "## User context" not in self_card
    # Pre-chip layout preserved.
    assert "## History" in self_card
