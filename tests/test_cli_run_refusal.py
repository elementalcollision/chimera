...
def _patch_git(monkeypatch, *, toplevel: str, branch: str,
               toplevel_rc: int = 0, branch_rc: int = 0) -> None:
    """Patch subprocess.run so the in-process drift detector sees the
    given (toplevel, branch) without touching the real repo state."""
...
def test_run_allowed_when_cwd_not_toplevel(isolated_run, monkeypatch):
    # toplevel reports a DIFFERENT path than cwd — sibling worktree.
    _patch_git(monkeypatch, toplevel="/some/other/path", branch="feat/chip-foo")
