...
def member_stats(label: str, verdicts: list, should_approve: list[bool]) -> MemberStats:
    ...
    (``.approved``) aligned with ``should_approve``."""
    correct = fa = fr = 0
    for v, ok in zip(verdicts, should_approve):
        if bool(v.approved) == ok:
            ...
            fr += 1
    return MemberStats(label, len(should_approve), correct, fa, fr)
...
def vote_agreement(a: list, b: list) -> float:
    ...
    independence signal for whether a candidate adds a fresh gradient to the panel."""
    pairs = list(zip(a, b))
    if not pairs:
        return 0.0
...
def panel_confusion(
    ...
    *,
    ...
    """Confusion of the panel decision (over ``per_member_verdicts`` — one verdict
    list per member) against the labels, using the real asymmetric panel rule."""
    correct = fa = fr = 0
    for i, ok in enumerate(should_approve):
        case_verdicts = [m[i] for m in per_member_verdicts]
        ...
        if approved == ok:
            ...
            fr += 1
    n = len(should_approve)
    return {"n": n, "correct": correct, "false_approve": fa, "false_reject": fr,
