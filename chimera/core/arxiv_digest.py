...
def write_digest_if_new(feed_path: Path, out_dir: Path, top_n: int = 12) -> dict:
    """Ingest the feed → write a dated digest into ``out_dir`` if one doesn't yet
    exist for this feed's run. Returns a fail-soft status dict (never raises)."""
    run_date, records = load_chimera_feed(feed_path)
    if not run_date or not records:
        return {"status": "no-feed", "run_date": run_date, "count": 0, "path": None}
    out = digest_path(out_dir, run_date)
    if out.exists():
        return {"status": "exists", "run_date": run_date,
                "count": len(records), "path": str(out)}
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(render_digest(records, run_date, top_n), encoding="utf-8")
    return {"status": "written", "run_date": run_date,
            "count": len(records), "path": str(out)}
