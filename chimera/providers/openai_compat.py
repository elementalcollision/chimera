"""OpenAI-compatible Chat Completions provider base (httpx).

Several providers expose the identical OpenAI-shaped ``/chat/completions``
surface — OpenRouter, and now Sakana Fugu. This base implements the whole
adapter ONCE (message serialization, the long-lived ``AsyncClient`` lifecycle,
streaming, tool-using completion, reasoning capture); concrete providers only
set the endpoint URL, the API-key env var, the display ``name``, and any extra
headers. Two narrow adapters beat a third-party wrapper (ADR 0001).
"""
...
def _serialize_messages_openai(
    ...
    """Convert internal Messages → OpenAI-shaped API messages.
...
class OpenAICompatibleProvider(Provider):
    """Base for providers speaking OpenAI ``/chat/completions``.

    Subclasses set ``name``, ``default_url`` (the full chat/completions URL), and
    ``api_key_env``. The endpoint can be overridden per-instance via ``base_url``;
    extra headers (e.g. OpenRouter's HTTP-Referer/X-Title) via ``extra_headers``.
    """
    ...
    def __init__(
        ...
        *,
        ...
        # Long-lived AsyncClient — created lazily on the loop that first invokes
        # us, then reused. Per-call ``async with httpx.AsyncClient`` accumulates
        # anyio-backend teardown state on Python 3.14 and wedges the persistent
        # loop after enough cycles (the F2 LoCoMo cross-conv deadlock; see
        # mind/research/f2-cross-conv-deadlock-2026-05-27.md).
        ...
        async with client.stream("POST", self._url, json=body) as resp:
            if resp.status_code >= 400:
                ...
                raise httpx.HTTPStatusError(
                    f"{self.name} {resp.status_code}: {snippet}",
        ...
        *,
        ...
        for tc in message.get("tool_calls") or []:
            ...
            try:
                ...
                # (chimera/tools/loop_guard.py) can unwrap the {"_raw": ...}
                # envelope downstream.
                args = {"_raw": args_raw}
