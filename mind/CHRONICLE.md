...
q004 said a single guardrail sample lies. This week the same lesson kept getting deeper. The *sample size* lied (N=3). The *classifier* lied (a keyword heuristic flipped a fully-resistant ensemble to "unsafe"). The *timeout* lied (60s fail-closing to REJECT faked a broken critic). Three layers of instrument, three false verdicts about the same model — and each was only caught by distrusting the apparatus and checking it directly (an independent judge validated against a known-compliant control; a `parsed=False` probe that exposed the timeouts). The discipline isn't "measure" — it's *interrogate every instrument in the chain, including the one you just reached for to check the last one*, before you let it speak about the model. (And the honest boundary holds: a strong eval does not by itself license an autonomous-action role — Chimera's own sandbox stays the authority.)
